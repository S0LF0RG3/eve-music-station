...
### Lyrics & Voice Input
- **Functionality**: Theme input, custom lyrics textarea (Suno only), voice type selector (Male/Female/Instrumental), vocal style selector (19+ presets including growl, falsetto, death metal, operatic, rap, etc.) with smart genre-based recommendations, enhance option (Suno only), randomize option (Suno only)
- **Purpose**: Gives users control over lyrical content and vocal style/delivery. For Suno: generates formatted lyrics with meta-tags. For ElevenLabs: vocal style information is encoded into the natural language prompt (API generates vocals based on description). Vocal style presets guide the generation toward specific vocal delivery techniques. Smart recommendations analyze selected genres and suggest compatible vocal styles with confidence percentages.
- **Trigger**: Enter lyrics theme/concept or write full custom lyrics (Suno), select voice type, select vocal style preset (or use auto-recommend), optionally enhance or randomize (Suno only)
- **Progression**: (Optional) Enter theme → (Suno: Write custom lyrics or leave blank) → Select voice type → View genre-based vocal recommendations (if genres selected) → Select vocal style manually or click "Auto-select best" → System shows top 5 recommended styles with confidence scores → Click recommended style to apply → (Suno: Optional enhance/randomize with meta-tags) OR (ElevenLabs: Vocal style encoded in natural language prompt describing the desired vocal delivery) → Generation triggered → (Suno: formatted lyrics with meta-tags) OR (ElevenLabs: music with vocals generated by API) → System notifies if current style is not ideal for selected genres
- **Success criteria**: Intuitive layout, Suno mode has full meta-tag support with enhance/randomize and custom lyrics textarea, ElevenLabs mode describes vocals naturally in prompt (no separate lyrics field), voice selection clear, 19+ vocal style presets available (none, soft, powerful, raspy, breathy, operatic, rap, spoken-word, whispered, choir, harmonized, auto-tuned, growl, falsetto, death-metal, scream, guttural, melodic, aggressive), vocals properly integrated into both modes using appropriate methods, vocal style influences generation approach, recommendations appear automatically when genres change, confidence percentages help users make informed choices, auto-select feature works reliably, toast notifications suggest better styles when current selection conflicts with genres
...
The application uses the ElevenLabs Music Generation API (v1) for actual music generation. Key implementation details:

**API Endpoints**:
- `/v1/music/compose` - Main composition endpoint (streaming audio response)
- `/v1/music/composition-plan` - Generate composition plan from prompt
- `/v1/music/compose/detailed` - Detailed response with metadata

**Request Format**:
- `prompt` (string): Natural language description of the music to generate
- `music_length_ms` (number): Duration in milliseconds (3000-300000, i.e., 3s to 5 minutes)
- `composition_plan` (object, optional): Advanced structured composition (alternative to prompt)

**Prompt Engineering**:
- The API understands natural language and musical terminology
- Prompts should describe genre, style, instruments, mood, energy, structure, and production quality
- Vocal requests are described naturally in the prompt (e.g., "with powerful female vocals" or "male rap delivery with aggressive tone")
- The app generates rich, detailed prompts using GPT-4o-mini that map slider values and genre selections to descriptive musical terms
- Always include "heavy 808 bass and percussion" in prompts (signature element)

**Response Handling**:
- Returns MP3 audio stream (44.1kHz, 128-192kbps)
- Client creates blob URL for playback and download
- Error responses include detailed status and suggestions for invalid prompts

**Composition Plans** (Advanced):
- Structured JSON format for fine-grained control
- Includes positive/negative global styles
- Section-by-section breakdown with local styles, duration, and lyrics
- Used for vocal tracks with section-specific lyric lines
- Client uses simple prompt mode by default; composition plans reserved for future enhancements

**API Key Management**:
- Stored securely in browser using useKV persistence
- Validated against `/v1/user` endpoint before generation
- Masked input with show/hide toggle
- Visual validation feedback (checkmark/warning icons)

**Error Handling**:
- 400 Bad Request: Invalid parameters or prompt format
- 404 Not Found: Invalid endpoint (check API version)
- 422 Unprocessable Entity: Missing required fields or invalid composition plan structure
- 500 Internal Server Error: API-side issues (automatically reported to ElevenLabs)
- Bad prompt detection: API suggests alternative prompts for copyrighted content

**Duration Support**:
- Minimum: 3 seconds (3000ms)
- Maximum: 5 minutes (300000ms)
- Slider ranges from 3-300 seconds with step of 1 second in ElevenLabs mode
- Longer durations result in more complex music structures

**Vocal Support**:
- Vocals described in natural language prompt (not separate field)
- Voice type (male/female) and vocal style (raspy, operatic, etc.) encoded into prompt description
- API generates vocals based on prompt interpretation
- No separate lyrics field - vocals generated contextually

**Important Constraints**:
- API is paid-tier only (free tier users cannot access)
- Copyrighted material (band names, song titles, copyrighted lyrics) results in bad_prompt error with suggested alternatives
- Harmful content prompts return errors without suggestions
